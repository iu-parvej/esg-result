...
const CustomTooltip = ({ active, payload, label }) => {
  if (active && payload && payload.length) {
    ...
    return (
      <div className="p-4 bg-white border border-gray-300 rounded-lg shadow-xl text-sm max-w-xs max-h-[60vh] overflow-y-scroll">
        <p className="font-bold text-gray-800 mb-2">{`CGPA ${data.rangeLabel}`}</p>
        ...
        <p className="text-gray-600 break-words">
          {data.students.length > 0 ? (
            data.students.map(student => student.roll).join(', ')
          ) : (
    ...
    const printContent = `
      ...
      <body>
        <div class="print-page">
          <div> <!-- Content wrapper for header, details, table -->
